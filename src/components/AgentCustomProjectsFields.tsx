      ...
      {enabled && (
        <div className="space-y-2">
          <p className="text-[11px] text-slate-500">
            官方商品来自平台模板。自定义商品只属于当前智能体。
          </p>
          {projects.map((item, index) => {
            const official = isOfficialProject(item);
            return (
              <div key={item.id} className="p-3 rounded-xl border border-slate-200 bg-white space-y-2">
                <div className="flex items-start gap-2">
                  {official ? (
                    <div className="flex-1 min-w-0">
                      ...
                      {item.description ? (
                        <p className="mt-1 text-[11px] text-slate-500 leading-5">{item.description}</p>
                      ) : null}
                    </div>
                  ) : (
                    <input
                ...
                <div className="flex items-center gap-3">
                  ...
                  {official && item.suggestedPrice ? (
                    <span className="text-[10px] text-slate-400">建议价 ¥{item.suggestedPrice}，仅供参考</span>
                  ) : null}
