...
import { CheckCircle2, XCircle, MessageSquare, Shield } from 'lucide-react';
  ...
  return (
    <div className="rounded-2xl border border-violet-200 bg-violet-50/50 p-4 space-y-4 text-xs">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="px-2 py-0.5 rounded-full bg-violet-100 text-violet-800 font-bold text-[10px]">待确认</span>
      </div>
      ...
      <div className="p-3 bg-white border border-violet-100 rounded-xl space-y-2">
        ...
        <AckCheckbox checked={ackAmount} onChange={setAckAmount} label={`我已确认订单金额 ${yuan(proposal.priceCents)}`} />
        <AckCheckbox checked={ackTime} onChange={setAckTime} label={`我已确认预计交付时间 ${proposal.deliveryDays} 天`} />
        <AckCheckbox checked={ackAcceptance} onChange={setAckAcceptance} label="我已确认验收标准" />
        <AckCheckbox checked={ackRevisions} onChange={setAckRevisions} label={`我已确认免费修改 ${proposal.freeRevisionCount} 次及验收规则`} />
        <AckCheckbox checked={ackEscrow} onChange={setAckEscrow} label="我已了解平台托管及争议处理规则" />
